...
public class NhanKhauController implements Initializable {
	...
	@FXML
	private TableColumn<NhanKhauModel, String> colTrangthai;
	@FXML
	...
	public void showNhanKhau() throws ClassNotFoundException, SQLException {
		...
		colTrangthai.setCellValueFactory(new PropertyValueFactory<NhanKhauModel, String>("trangthai"));
		colGioitinh.setCellValueFactory(new PropertyValueFactory<NhanKhauModel, String>("gioitinh"));
		colMaHo.setCellValueFactory(new PropertyValueFactory<NhanKhauModel, String>("maho"));
//		try {
...
//		}
		...
		ObservableList<String> listComboBox = FXCollections.observableArrayList("Tên", "Tuổi", "id", "Tạm trú", "Có mặt", "Mã hộ", "Tạm vắng");
	...
	public void searchNhanKhau() {
		...
		case "Mã hộ": {
			...
			Pattern pattern = Pattern.compile("\\d{1,}");
			if(!pattern.matcher(keySearch).matches()) {
				Alert alert = new Alert(AlertType.WARNING, "Mã hộ nhập vào phải là 1 số!", ButtonType.OK);
				alert.setHeaderText(null);
				alert.showAndWait();
				break;
			}
			...
			int index = 0;
			List<NhanKhauModel> listNhanKhau_tmp4 = new ArrayList<>();
			for (NhanKhauModel nhanKhauModel : listNhanKhau) {
				if (nhanKhauModel.getMaho() == Integer.parseInt(keySearch)) {
					listNhanKhau_tmp4.add(nhanKhauModel);
					index++;
				}
			}
			listValueTableView_tmp = FXCollections.observableArrayList(listNhanKhau_tmp4);
			tvNhanKhau.setItems(listValueTableView_tmp);
		...
		case "Vắng mặt": {
			...
			int index = 0;
			List<NhanKhauModel> listNhanKhau_tmp2 = new ArrayList<>();
			for (NhanKhauModel nhanKhauModel : listNhanKhau) {
				if (nhanKhauModel.getTrangthai().contains(key)) {
					listNhanKhau_tmp2.add(nhanKhauModel);
					index++;
				}
			}
			listValueTableView_tmp = FXCollections.observableArrayList(listNhanKhau_tmp2);
			tvNhanKhau.setItems(listValueTableView_tmp);
		...
		case "Có mặt": {
			...
			int index = 0;
			List<NhanKhauModel> listNhanKhau_tmp3 = new ArrayList<>();
			for (NhanKhauModel nhanKhauModel : listNhanKhau) {
				if (nhanKhauModel.getTrangthai().contains(key)) {
					listNhanKhau_tmp3.add(nhanKhauModel);
					index++;
				}
			}
			listValueTableView_tmp = FXCollections.observableArrayList(listNhanKhau_tmp3);
			tvNhanKhau.setItems(listValueTableView_tmp);
	...
	public void addNhanKhau(ActionEvent event) throws IOException, ClassNotFoundException, SQLException {
        ...
        stage.setScene(new Scene(home,800,700));
	...
	public void updateNhanKhau() throws IOException, ClassNotFoundException, SQLException {
        ...
        Stage stage = new Stage();
        stage.setScene(new Scene(home,700,700));
