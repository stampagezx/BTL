...
public class NopTienController implements Initializable {
	...
	@FXML
	private TableColumn<NopTienModel, String> tbcIDNopTien;
	@FXML
	...
	Map<Integer, String> mapmahoToTen;
	...
	public void showNopTien() throws ClassNotFoundException, SQLException {
		...
		mapmahoToTen = new HashMap<>();
		listHoKhau.forEach(hokhau -> {
			mapmahoToTen.put(hokhau.getMaHo(), hokhau.getTenChuHo());
		});
		...
		try {
			tbcTenNguoi.setCellValueFactory((CellDataFeatures<NopTienModel, String> p) -> new ReadOnlyStringWrapper(
					mapmahoToTen.get(p.getValue().getIdNopTien())));
		} catch (Exception e) {
		...
		tbcIDNopTien.setCellValueFactory(new PropertyValueFactory<>("idNopTien"));
	...
	public void searchNopTien() {
		...
		case "Tên người nộp": {
			...
			for (NopTienModel nopTienModel : listNopTien) {
				if (mapmahoToTen.get(nopTienModel.getIdNopTien()).contains(keySearch)) {
		...
		default: { // truong hop con lai : tim theo ten khoan thu
			...
			List<NopTienModel> listNopTienModelsSearch = new ArrayList<>();
			for (NopTienModel nopTienModel : listNopTien) {
				if (mapIdToTenKhoanThu.get(nopTienModel.getMaKhoanThu()).contains(keySearch)) {
					listNopTienModelsSearch.add(nopTienModel);
					index++;
				}
			}
